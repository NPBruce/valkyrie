...
function Build-Unity {
    ...
    Write-Log "Waiting for unity sub-processes to complete..."
    while ((Get-CimInstance -Class Win32_Process | Where-Object { $_.ParentProcessID -eq $UnityProcess.Id -and $_.Name -ne 'VBCSCompiler.exe' }).count -gt 0) {
    ...
    Write-Log "Waiting for unity sub-processes to complete finished."
