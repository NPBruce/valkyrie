...
public class EditorComponentMonsterPlacement : EditorComponent
{
    ...
    override public void Update()
    {
        ...
        TextButton tb = new TextButton(new Vector2(0, 0), new Vector2(4, 1), 
            CommonStringKeys.MONSTER, delegate { QuestEditorData.TypeSelect(); });
        tb.button.GetComponent<UnityEngine.UI.Text>().fontSize = UIScaler.GetSmallFont();
        tb.button.GetComponent<UnityEngine.UI.Text>().alignment = TextAnchor.MiddleRight;
        tb.ApplyTag("editor");

        tb = new TextButton(new Vector2(4, 0), new Vector2(15, 1), 
            new StringKey(name.Substring("Monster".Length),false), 
            delegate { QuestEditorData.ListMonster(); });
        tb.button.GetComponent<UnityEngine.UI.Text>().fontSize = UIScaler.GetSmallFont();
        ...
        for (int heroes = 2; heroes < 5; heroes++)
        {
            ...
            tb = new TextButton(new Vector2(19, offset++), new Vector2(1, 1), CommonStringKeys.PLUS, 
                delegate { MonsterPlaceAdd(h); }, Color.green);
            tb.button.GetComponent<UnityEngine.UI.Text>().fontSize = UIScaler.GetSmallFont();
            tb.ApplyTag("editor");
    ...
    public void MonsterPlaceAdd(int heroes)
    {
        ...
        monsterPlaceESL = new EditorSelectionList(
            new StringKey("val","SELECT", CommonStringKeys.ITEM), 
            mplaces, delegate { MonsterPlaceAddSelection(heroes); });
