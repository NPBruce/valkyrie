...
public class HeroSelection {
    ...
    public void Draw()
    {
        ...
        foreach (string hero in heroList)
        {
            ...
            if (left)
            {
                tb = new TextButton(new Vector2(5f, offset), new Vector2(4.25f, 4.25f), 
                    StringKey.NULL, delegate { Select(hero); });
            }
            else
            {
                tb = new TextButton(new Vector2(UIScaler.GetWidthUnits() - 7.25f, offset), new Vector2(4.25f, 4.25f), 
                    StringKey.NULL, delegate { Select(hero); });
            }
            ...
            if (left)
            {
                tb = new TextButton(new Vector2(9.25f, offset + 1.5f), new Vector2(UIScaler.GetWidthUnits() - 19, 1.5f),
                    StringKey.NULL, delegate { Select(hero); }, Color.clear);
            }
            else
            {
                tb = new TextButton(new Vector2(11.75f, offset + 1.5f), new Vector2(UIScaler.GetWidthUnits() - 20, 1.5f), 
                    StringKey.NULL, delegate { Select(hero); }, Color.clear);
            }
            tb.background.GetComponent<UnityEngine.UI.Image>().color = Color.white;
            tb.background.transform.parent = scrollArea.transform;
