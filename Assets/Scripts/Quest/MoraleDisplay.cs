...
public class MoraleDisplay {
    ...
    {
        Game game = Game.Get();
        md = new DialogBox(new Vector2(0.75f, 0.5f), new Vector2(3, 3), game.round.morale.ToString(), Color.red);
    ...
    {
        Game game = Game.Get();
        md.textObj.GetComponent<UnityEngine.UI.Text>().text = game.round.morale.ToString();
    }
}
